...
g_mic =reshape(g,Lg,[]); % g_mic is the coefficient 
...
leng = samplesToKeep; %length(filter(computed_rir.RIR_sources(:,1,1),1,source_speech{1,1}));
...
if(flag_output == 1 || flag_output == 3)
	...
	for i = 1:1:numOfMics
		...
		if(numOfNoiseSources > 0)
			noise = fftfilt(computed_rir.RIR_noise(:,i,1),source_noise{1,1},leng); %% noise signal is filtered by the Room IR
		end
		if(numOfNoiseSources > 1)
			for k = 2:1:numOfNoiseSources
				noise = noise + fftfilt(computed_rir.RIR_noise(:,i,k),source_noise{k,1},leng); % each mic received a weighted sum of noise signal. Weights are the RIR
			end
		end 
